<script setup>
import { ref } from "vue";
import {generateMultiplicationChallenge, submitMultiplicationChallenge} from "@/functions/multiplicationFunctions.js";
  ...
  <div style="width: 30vw; display: flex; align-content: center; flex-direction: column">
    <v-sheet id="generate-challenge">
      <v-form @submit.prevent="(userName !== '' && difficulty >= 1 && difficulty <= 10) ?
            generateMultiplicationChallenge(userName, difficulty, challenge) : null">
        ...
        <v-btn type="submit"> Generate Multiplication Challenge </v-btn>
      </v-form>
    ...
    <v-sheet id="display-challenge">
      ...
      <div v-if="challenge.value != null">
        ...
        <h1>{{challenge.value.num1}} X {{challenge.value.num2}}</h1>
      </div>
      <div v-else>
        <p>No challenge generated yet</p>
      </div>
      <v-form @submit.prevent="submitMultiplicationChallenge(challenge.value, userAnswer, submitResponse)">
